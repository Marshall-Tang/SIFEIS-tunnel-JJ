...
rPerm_FE = np.array([100, 200, 400, 800])   # relative permittivity of FE
d = np.linspace(1, 4, 51) * 1e-9            # FE thickness
...
# ----------- Folders Setup -----------
RESULTS_FOLDER = Path("results/FE_dependence")
...
def main():
    '''
    Simulate the efficiency (eta) as a function of the FE thickness (d) and with different dielectric constants (rPerm_FE).
    '''
    print("Running experiment 'FE_dependence'...")
    t0 = time.time()
    # Generate and save data
    data_file_name = process.Data_d_dependence(
        Ef, P, l_1, l_2, d, delta, U_0, U_1, U_2, rPerm_FE, Gap_Sc, DATA_FOLDER)
    # Plot and save figures
    process.Plot_d_dependence(data_file_name, FIG_FOLDER)
